...
def printProgressBar (iteration, total, prefix = '', suffix = '', decimals = 1, length = 500, fill = 'ጿ ኈ ቼ ዽ '):
    """
...
def compute_mrcnn_mask_loss(target_masks, target_class_ids, pred_masks):
    ...
    if target_class_ids.size():
        ...
        # loss = dice_coeff(pred=y_pred, target=y_true)
        #
...
class MaskRCNN(nn.Module):
    ...
    def set_log_dir(self, model_path=None):
        ...
        if os.path.exists(os.path.join(self.model_dir, "{}{:%Y%m%dT%H%M}".format(
            self.config.NAME.lower(), now))):
            self.log_dir = os.path.join(self.model_dir, "{}{:%Y%m%dT%H%M}".format(
                self.config.NAME.lower(), now))
        else:
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        layer_regex = {
            ...
            "1+": r"(fpn.C1.*)|(fpn.C2.*)|(fpn.C3.*)|(fpn.C4.*)|(fpn.C5.*)|(fpn.P5\_.*)|(fpn.P4\_.*)|(fpn.P3\_.*)|(fpn.P2\_.*)|(rpn.*)|(classifier.*)|(mask.*)",
            "2+": r"(fpn.C2.*)|(fpn.C3.*)|(fpn.C4.*)|(fpn.C5.*)|(fpn.P5\_.*)|(fpn.P4\_.*)|(fpn.P3\_.*)|(fpn.P2\_.*)|(rpn.*)|(classifier.*)|(mask.*)",
