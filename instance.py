...
def main():
    ...
    parser.add_argument("--labels", help="labels file",
                        default='./mydata/labels.txt')  # required=True
    ...
    for i, line in enumerate(open(args.labels).readlines()):
        ...
        data["categories"].append(
            dict(supercategory="fastener", id=class_id, name=class_name, )  # 一类目标+背景，id=0表示背景
        )
        data["categories"].append(
            dict(supercategory="hat", id=class_id, name=class_name, )  # 一类目标+背景，id=0表示背景
        )
        data["categories"].append(
            dict(supercategory="shim", id=class_id, name=class_name, )  # 一类目标+背景，id=0表示背景
        )
    print("categories 生成完成", '\n')

    out_ann_file = osp.join(args.output_dir, "track_train_anno.json")  # 自动添加"/"   这里要改
